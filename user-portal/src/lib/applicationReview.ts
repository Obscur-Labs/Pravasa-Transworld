...
export type ReviewRow =
  ...
  | { kind: 'missing'; id: string; label: string; requirementName: string }
...
// Mirrored here (order -1) so those front/back scans lead the list, exactly as they
// led the form, instead of falling to the bottom as unrecognised extras.
const DEFAULT_PASSPORT: DocumentRequirement = {
  _id: '__passport_default__', name: 'Passport', description: '', required: true,
  applicantType: 'both', docType: 'passport', order: -1,
};
...
/** Answers belonging to the trip as a whole (travel dates, …) rather than one traveller. */
...
export function buildReviewRows(
  ...
  // Passport details read by OCR are shown under the scan they came from, and were also
  // copied into the answers at submit time. Drop that copy so each value appears once.
  ...
  for (const item of mergeFormItems(fields, reqs)) {
    if (item.kind === 'document') {
      ...
      if (matches.length === 0 && item.doc.required) {
        rows.push({
          kind: 'missing',
          id: `missing:${item.doc.name}`,
          label: item.doc.name,
          // The name a replacement upload has to be filed under to land in this slot.
          requirementName: traveler ? `${traveler} - ${item.doc.name}` : item.doc.name,
        });
      }
    } else {
  ...
  // applicant added, questions since removed) still have to be shown.
